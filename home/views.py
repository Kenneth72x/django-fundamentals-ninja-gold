...
# helper dictionary, for easy access to min/max gold coin values
GOLD_MAP = {
    "farm": (10,20),
    "cave": (5,10), 
    "house": (2,5),
...
def process_gold(request):
    ...
    # access the correct min/max values from the user's form submission     
    building = GOLD_MAP[building_name]
    # uppercase string (for message)
    ...
    # generate a datetime string, with the proper format, for the RIGHT NOW
    ...
    if building_name == 'casino':
        ...
        if random.randint(0,1) > 0: # 50% chance of being true/false
            ...
            message = f"Entered a {building_name_upper} and lost {curr_gold} gold coins... cheeeeee....how lame haha! ({now_formatted})"
    ...
    # update session activites with new message
